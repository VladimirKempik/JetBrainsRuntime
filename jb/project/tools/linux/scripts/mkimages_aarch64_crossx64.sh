...
function do_configure_buildjdk {
  sh configure \
    --with-vendor-name="$VENDOR_NAME" \
    --with-boot-jdk="$BOOT_JDK" \
    --with-jvm-features="-zgc,-compiler2,-epsilongc,-g1gc,-jfr,-jni-check,-jvmci,-jvmti,-shenandoahgc" \
    --with-conf-name="$BUILD_JDK_CONF_NAME" \
    || do_exit $?
}
...
#build build-jdk for native platform first

if [ -z "${INC_BUILD:-}" ]; then
  do_configure_buildjdk || do_exit $?
  make clean CONF=$BUILD_JDK_CONF_NAME || do_exit $?
fi

make images CONF=$BUILD_JDK_CONF_NAME || do_exit $?

BUILD_JDK="build/$BUILD_JDK_CONF_NAME/images/jdk"
